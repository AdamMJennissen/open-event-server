...
class SpeakersCallList(ResourceList):
    ...
    def before_create_object(self, data, view_kwargs):
        ...
        try:
            speakers_call = (
                self.session.query(SpeakersCall)
    ...
    def before_get_object(self, view_kwargs):
        ...
        if view_kwargs.get('event_identifier'):
            try:
                event = (
                    ...
                    .filter_by(identifier=view_kwargs['event_identifier'])
                    .one()
                )
            except NoResultFound:
        ...
        if view_kwargs.get('event_id'):
            try:
                speakers_call = (
                    self.session.query(SpeakersCall)
                    .filter_by(event_id=view_kwargs['event_id'])
                    .one()
                )
            except NoResultFound:
                raise ObjectNotFound(
...
class SpeakersCallRelationship(ResourceRelationship):
    ...
    schema = SpeakersCallSchema
    methods = ['GET', 'PATCH']
    data_layer = {'session': db.session, 'model': SpeakersCall}
